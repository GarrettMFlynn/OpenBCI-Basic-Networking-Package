...
{
    public static BandsInlet inlet3;
    ...
    void Start()
    {
        ...
        {
            GameObject _instanceSampleCube = GameObject.CreatePrimitive(PrimitiveType.Cube);//(GameObject)Instantiate(_sampleCubePrefab);
    ...
    void Update()
    {
        ...
        for (int i = 0; i < _sampleCube.Length; i++)
        {
            if (_sampleCube[i] != null)
            {
                ...
                _sampleCube[i].transform.localScale = new Vector3(10, (float)height, 10);
            }
        }
    }
}
